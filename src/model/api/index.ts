...
const getBalance = async (publicKey: string) => {
  ...
  if (typeof lamports === 'number') {
    ...
    console.log(sol); // Now you can safely use the result
  } else {
    console.error('lamports is not a valid number.');
  }
...
const getHistory = async (publicKeyString: string, options = {limit: 20}) => {
  const connection = createConnection();
